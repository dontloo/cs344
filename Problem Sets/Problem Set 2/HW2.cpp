...
void preProcess(uchar4 **h_inputImageRGBA, uchar4 **h_outputImageRGBA,
  ...
  cv::Mat image = cv::imread(filename.c_str(), cv::ImreadModes::IMREAD_COLOR);
  if (image.empty()) {
    std::cerr << "Couldn't open file: " << filename << std::endl;
    exit(1);
  }

  cv::cvtColor(image, imageInputRGBA, cv::ColorConversionCodes::COLOR_BGR2RGBA);
...
void postProcess(const std::string& output_file, uchar4* data_ptr) {
  ...
  cv::cvtColor(output, imageOutputBGR, cv::ColorConversionCodes::COLOR_RGBA2BGR);
  //output the image
